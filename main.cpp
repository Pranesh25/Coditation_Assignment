...
int colPrint(int col){                                                           //used to print column lines for decoration or neat view of grid
    cout<<endl;
    for(int i=0; i<col; i++)
    {
        cout<<" -----";
...
void getFinalGrid(int initialGrid[][COL] ,int finalGrid[][COL],int row,int col)                                          //to get the final gird .
{
    ...
    for(int i=0; i<row; i++)
    {
        for(int j=0;j<col;j++)
        {
            ...
            if(initialGrid[i][j]==1 && (neighbourLiveCell==2 || neighbourLiveCell==3))                                  // to mark the conditions given in the assignment
            {
                finalGrid[i][j]=1;                                                                                      // we are finalising which grid is live and which one is dead according to 
            }
...
void checkCell(int finalGrid[][COL],int newRow,int newCol,int row, int col){
    ...
    if(newRow<row && newCol<col)
	{
		...
		{
    		cout<<"Cell at "<<newRow<<" "<<newCol<<" is "<<"Dead"<<endl;            // to check cell if it is alive or dead
    	}
    }
	else
	{
...
int main(){                                                                      //main function
    ...
    cout<< "Enter the number of rows and  number of colomns you want to create \n max  limit 500 \n" <<endl;
    ...
    cout<<" \n To know current state of cell ,Enter its Row and column :"<<endl;                                   
    cin>>newRow>>newCol;
    cout<<endl;
    
    checkCell(finalGrid,newRow,newCol,row,col);
    cout<<" \n do you want to continue \n press 1 for continue  \t press 0 to exit"<<endl;
    int ans;
    cin >>ans;
    cout<<endl;
    if(ans == 0)
